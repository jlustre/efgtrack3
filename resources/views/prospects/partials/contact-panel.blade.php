...
<div class="rounded-lg border border-slate-400 bg-white p-6 shadow-sm lg:col-span-1">
    <h2 class="text-lg font-semibold text-[#0B1F3A]">Contact</h2>
    ...
    @if ($prospect->occupation || $prospect->employer_business || $prospect->date_of_birth || $prospect->gender || $prospect->marital_status || $prospect->children_count !== null)
        <div class="mt-6 border-t border-slate-200 pt-4">
            <h3 class="text-sm font-semibold text-[#0B1F3A]">Personal Profile</h3>
            <dl class="mt-3 space-y-3 text-sm">
                @if ($prospect->occupation)
                    <div><dt class="font-semibold text-slate-500">Occupation</dt><dd class="text-[#0B1F3A]">{{ $prospect->occupation }}</dd></div>
                @endif
                ...
                @endif
                @if ($prospect->gender)
                    <div><dt class="font-semibold text-slate-500">Gender</dt><dd class="text-[#0B1F3A]">{{ $prospect->gender }}</dd></div>
                @endif
                @if ($prospect->marital_status)
                    <div><dt class="font-semibold text-slate-500">Marital Status</dt><dd class="text-[#0B1F3A]">{{ $prospect->marital_status }}</dd></div>
                @endif
                @if ($prospect->children_count !== null)
                    <div><dt class="font-semibold text-slate-500">Children</dt><dd class="text-[#0B1F3A]">{{ $prospect->children_count }}</dd></div>
                @endif
            </dl>
        </div>
    @endif
    ...
    @if ($prospect->notes_summary)
        <div class="mt-6 border-t border-slate-200 pt-4">
            <h3 class="text-sm font-semibold text-slate-700">Summary</h3>
            <p class="mt-2 text-sm text-slate-600">{{ $prospect->notes_summary }}</p>
        </div>
    @endif
</div>
